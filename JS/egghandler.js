...
function updateEggPage() {
    ...
    if(DOMCacheGetOrSet('currentEggImg').getAttribute('src') !== `Imgs/${eggImgIDs[data.currentEgg]}.png`) 
        ...
        DOMCacheGetOrSet('currentEggText').innerHTML = `Current Egg: ${eggNames[data.currentEgg]}<br>Value: $${format(eggValue[data.currentEgg])} (x${format(eggValueBonus)})<br>Egg Laying Rate: +${format(layRate.sub(1))}%<br>Chicken Gain: ${format(chickenGain)} Chickens/min`
